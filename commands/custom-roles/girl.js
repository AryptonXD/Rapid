    ...
    run: async (client, message, args) => {
     ...
     const rajaa = new MessageEmbed()
      ...
      .setFooter(client.user.tag, client.user.displayAvatarURL())
        ...
        if(!args[0]){
            return message.channel.send({embeds : [new MessageEmbed().setColor(`#2f3136`).setDescription(`Usage: ${prefix}girl <user>`)]})
        }
        ...
        let gRole = await client.db3.get(`girl_${message.guild.id}`);
        if(!gRole || gRole == null){
            return message.channel.send({embeds : [new MessageEmbed().setColor(`#2f3136`).setDescription(`<:ET_cross:1022870496250953769> | **Girl Role** not found.`)]})
        }
