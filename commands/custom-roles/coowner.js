    ...
    run: async (client, message, args) => {
     ...
     const rajaa = new MessageEmbed()
      ...
      .setFooter(client.user.tag, client.user.displayAvatarURL())
        ...
        if(!args[0]){
            return message.channel.send({embeds : [new MessageEmbed().setColor(`#2f3136`).setDescription(`Usage: ${prefix}coowner <user>`)]})
        }
        ...
        let gRole = await client.db3db.get(`coowner_${message.guild.id}`);
        if(!gRole || gRole == null){
            return message.channel.send({embeds : [new MessageEmbed().setColor(`#2f3136`).setDescription(`<:ET_cross:1022870496250953769> | **Co-Owner Role** not found.`)]})
        }

        if(!message.guild.roles.cache.has(gRole)){
            await client.db3db.set(`friend_${message.guild.id}`,null);
        ...
        return message.channel.send({embeds : [new MessageEmbed().setColor(`#2f3136`).setDescription(`<:ET_Tick:1020380251529937007> | Successfully Added <@&${gRole}> to ${abc}`)]});
       		})
        })
    }
}